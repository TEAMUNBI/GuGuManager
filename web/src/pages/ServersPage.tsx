...
function CreateServerModal({ open, games, nodes, csrfToken, onClose, onCreated, copy }: { open: boolean; games: GameDefinition[]; nodes: Node[]; csrfToken: string; onClose: () => void; onCreated: (operation: Operation) => Promise<void>; copy: ServersCopy }) {
  ...
  const [gameID, setGameID] = useState(games.find((game) => game.status === "approved")?.id ?? "");
  const [nodeID, setNodeID] = useState(nodes.find((node) => node.condition === "available")?.id ?? "");
  const [memory, setMemory] = useState(4096);
  const [disk, setDisk] = useState(25);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  useEffect(() => { if (games.length && !gameID) setGameID(games[0].id); if (nodes.length && !nodeID) setNodeID(nodes[0].id); }, [gameID, games, nodeID, nodes]);
  const selectedGame = games.find((game) => game.id === gameID);
  const submit = async (event: React.FormEvent) => {
    event.preventDefault(); setBusy(true); setError("");
    try {
      if (!selectedGame) throw new Error(copy.gameMissing);
  ...
  return <Modal open={open} title={copy.createTitle} description={copy.createDescription} onClose={onClose} footer={<><button className="button secondary" type="button" onClick={onClose}>{copy.cancel}</button><button className="button primary" form="create-server-form" disabled={busy || !name || !gameID || !nodeID}>{busy ? copy.provisioning : copy.create}<ChevronRight size={16} /></button></>}><form id="create-server-form" className="modal-form" onSubmit={submit}><label>{copy.gameRelease}<select value={gameID} onChange={(event) => setGameID(event.target.value)}>{games.filter((game) => game.status === "approved").map((game) => <option key={game.id} value={game.id}>{game.name} {game.gameVersion} / {copy.definition} {game.version}</option>)}</select>{selectedGame && <small className="field-hint">{localizedGameSummary(selectedGame, locale)}</small>}</label><label>{copy.serverName}<input value={name} onChange={(event) => setName(event.target.value)} placeholder={copy.namePlaceholder} autoComplete="off" required /></label><label>{copy.node}<select value={nodeID} onChange={(event) => setNodeID(event.target.value)}>{nodes.map((node) => <option key={node.id} value={node.id} disabled={node.condition !== "available"}>{node.name} / {nodeLabel[node.condition]}</option>)}</select></label><div className="form-two-col"><label>{copy.memoryMb}<input type="number" min={512} max={131072} step={256} value={memory} onChange={(event) => setMemory(Number(event.target.value))} /></label><label>{copy.diskGb}<input type="number" min={1} max={2048} value={disk} onChange={(event) => setDisk(Number(event.target.value))} /></label></div>{error && <div className="form-error" role="alert">{error}</div>}</form></Modal>;
}
