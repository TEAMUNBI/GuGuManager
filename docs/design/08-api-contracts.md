...
`api/openapi/openapi.yaml` 是当前开发垂直切片 REST API 的机器契约。它不表示生产 MVP 已经实现。本章同时记录生产目标；目标端点只有进入 OpenAPI、实现和契约测试后才可视为可用。
...
开发专用的 `/api/v1/dev/*` 端点会在 OpenAPI 中单独标记为 development-only，以便当前 Web、测试和本地 Agent 适配器共享准确契约；它们不提供稳定性或生产兼容承诺。生产 Agent transport 使用 mTLS/gRPC，不沿用开发 Bearer HTTP 路由。
...
当前开发切片只有 `/servers` 声明 cursor 和 `limit`，内存数据集固定返回 `nextCursor: null`；其他开发列表返回 `{ "data": [...] }`。生产 MVP 中所有可能无界增长的列表都使用不透明 cursor：`limit` 默认 25、最大 100，响应提供 `nextCursor`，排序使用唯一 ID 作为最终稳定键。
...
| 电源 | `POST /servers/{id}/power` | 模拟 `start`、`stop`、`restart`、`kill` 的异步收敛 |
| Operation | `GET /operations/{id}` | 返回 `serverId`、受理时不可变的 `nodeId` 节点快照、状态、进度、generation、attempt、lease、checkpoint、结构化错误和时间；查询按 `serverId` 重新授权，当前执行语义仅来自内存开发 worker |
| 控制台 | `GET /servers/{id}/console`、`POST /servers/{id}/console/commands` | REST 快照与内存命令回显；不是 WebSocket |
| 文件 | `/servers/{id}/files`、`/files/content`、`/files/directories`、`/files/moves` | 对开发服务器数据根执行安全列表、读写、建目录、移动和删除；不是 Agent 文件传输 |
| 备份 | `GET/POST /servers/{id}/backups`、`POST /servers/{id}/backups/{backupId}/restore`、`DELETE /servers/{id}/backups/{backupId}` | 模拟创建、停服恢复和异步删除，具有幂等、摘要登记与恢复锁；没有真实备份制品 |
| Network | `GET/POST /servers/{id}/allocations`、`PATCH/DELETE /servers/{id}/allocations/{allocationId}` | 内存 Allocation 增删与切主，使用 CSRF、幂等键、generation fencing 和模拟 `reconcile`；不绑定真实端口 |
| Startup | `GET/PUT /servers/{id}/startup` | 从服务器固定 Bundle 读取命令和受限变量 Schema，内存更新后模拟 `reconcile`；Secret 只返回 `hasValue` 等声明状态 |
| 节点 | `GET /nodes` | 查询种子节点和开发心跳结果；30 秒无心跳转为 offline 并拒绝新任务 |
| 开发 Agent | `POST /dev/agent/heartbeat` | 使用 development Agent Token 上报种子节点心跳；仅供开发适配器，不是生产 Agent 协议 |
| 游戏目录 | `GET /game-definitions` | 查询开发游戏定义和固定 Bundle 摘要 |
| 审计 | `GET /audit-events` | 查询进程内审计记录；setup、用户、重置、membership 和异步服务器操作均记录开发审计 |

上述 Identity 路由已有开发 Web 页面：首次 setup、本地用户与角色管理、一次性密码重置令牌签发和消费，以及 server membership 管理。所有数据与令牌摘要仍保存在单进程内存。当前仍未实现 PostgreSQL/Redis Identity 适配器、持久会话、多副本一致撤销、实时连接撤销、节点注册令牌、CSR 注册、console token、WebSocket、Agent 文件传输、真实备份制品、真实端口绑定和加密 Secret 存储。调用这些目标能力不得回退到模拟成功。

## 4. 生产 MVP 目标矩阵

| 能力 | 目标端点或协议 | 状态 |
| --- | --- | --- |
| 初始化 | `GET /setup/status`、`POST /setup/admin` | 已有开发内存接口和单次 Bootstrap 摘要校验；部署 Secret 管理、PostgreSQL 原子状态和多副本关闭未实现 |
| 登录与会话 | `/auth/login`、`/auth/session`、`/auth/logout` | 已有 Argon2id、本地用户、内存 Token 摘要、账号/IP 限流和撤销；持久会话、密钥轮换与多副本撤销未实现 |
| 用户与成员 | `/users`、`/users/{userId}`、`/users/{userId}/password-reset-tokens`、`POST /auth/password-reset`、`/servers/{serverId}/members/{userId}` | 已有内存接口、开发管理页面、单次重置、会话撤销和 REST 资源授权；PostgreSQL 事务、生产持久化、跨副本和实时连接撤销未实现 |
| 注册令牌 | `POST /node-enrollment-tokens` | 未实现；一次性、短 TTL、明文只返回一次 |
| Agent 注册 | `POST /agent/enroll` 或版本化 Agent 注册服务 | 未实现；消费令牌、校验 CSR 并签发短期证书 |
| 节点 | `/nodes` | 仅有开发查询；维护模式、吊销和能力协商未实现 |
| Bundle | `/game-definitions` 与版本化 Bundle 资源 | 仅有开发查询；校验、审核、安装和签名验证未实现 |
| 服务器 | `/servers`、`/servers/{id}` | 仅有模拟创建与查询；生产事务、Allocation 和 OCI provision 未实现 |
| Network | `/servers/{id}/allocations`、`/servers/{id}/allocations/{allocationId}` | 已有开发内存接口；PostgreSQL 事务、`portRef`、多端口 Bundle 映射、权限和真实端口绑定未实现 |
| Startup | `/servers/{id}/startup` | 已有固定 Bundle Schema 驱动的开发内存接口；加密持久化、Secret 句柄、权限和 Agent 对账未实现 |
| 电源 | `/servers/{id}/power` | 仅有模拟 operation；Agent 投递与真实 Runtime 未实现 |
| Operation | `/operations/{id}` | 当前已有必填 `nodeId` 执行节点快照、单次 attempt、短暂内存 lease、checkpoint、结构化错误和按 `serverId` 的读取授权；持久租约、重试、Worker/Outbox 与跨副本恢复未实现 |
| 实时控制台 | `POST /servers/{id}/console-token` 与 WebSocket | 未实现；当前仅有 REST 快照和开发命令回显 |
| 文件 | `/servers/{id}/files` 的读写方法 | 已实现开发本地数据根的读写、移动和删除；Agent 传输、流式上传/下载、解压与配额未实现 |
| 备份 | `/servers/{id}/backups` 及 restore/delete 行为 | 已实现开发状态机、摘要登记、恢复锁和删除；真实归档、内容读取与恢复结果校验未实现 |
| 审计 | `/audit-events` | 仅有开发查询；持久化、过滤、分页和保留策略未实现 |

生产目标端点进入 OpenAPI 时必须同时补认证、权限、错误响应、幂等要求和契约测试，不能只增加路由占位符。

备份处于 `creating`、`failed`、`restoring` 或 `deleting` 时，`sizeBytes` 与 `checksum` 可以缺省或为 `null`；只有 `ready` 备份可以用于恢复，且此时二者必须存在、摘要必须是完整 `sha256`。当前开发适配器只在模拟创建收敛后插入带完整元数据的备份记录。未来持久 Store 映射数据库 `NULL` 时必须使用可空表示或省略字段，不能把大小映射为 `0`、把摘要映射为空字符串。

## 5. 身份、会话与授权边界

当前登录失败统一返回 `AUTH_INVALID_CREDENTIALS`，不泄露账号存在性。Cookie 会话、用户、角色、setup、reset token 和 membership 都由开发内存适配器保存；默认快速启动使用已初始化的种子管理员，未初始化构造路径用于 setup 契约和本地开发流程。
...
生产会话必须通过 `Secure`、`HttpOnly`、`SameSite=Lax` Cookie 传递；非安全方法需要 CSRF Token，WebSocket 还必须校验 Origin 和短时连接令牌。生产 Identity 必须把用户、令牌、会话、setup 和 membership 接入 PostgreSQL，并实现多副本撤销、密钥轮换、审计保留和实时连接失效；不得把当前进程内行为视为这些目标已经完成。

## 6. 异步操作、幂等与并发

当前开发切片的服务器创建、电源、备份创建、备份恢复、备份删除、Allocation 写入和 Startup 更新要求 `Idempotency-Key`，长度为 16 至 128 个可打印 ASCII 字符。同一 actor、路由作用域、键和请求摘要返回原 operation；同键不同请求返回 `409 IDEMPOTENCY_KEY_REUSED`。Startup 摘要包含 Secret，但使用每进程随机密钥的 HMAC-SHA256，不保存普通 SHA-256 指纹。内存幂等记录和密钥随进程退出而丢失。
...
上述契约不构成持久任务执行保证。进程退出会同时丢失 operation、lease、checkpoint 和幂等记录；不存在数据库原子领租、续租、过期重领、退避重试、Outbox 投递、Agent 检查点回报或多副本一致性。生产客户端不得把固定 30 秒 lease 当作任务超时、互斥锁或重试依据。
